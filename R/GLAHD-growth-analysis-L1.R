...
#Did provenances respond similarly across groups?
#Treatment increased final biomass in S not N and sp within 
...
#- home photo
ylims=c(0,90)
boxplot(predMass~Treatment+Taxa,data=T60,ylim=ylims,ylab=expression(Mass~(g)),axes=F,las=2,col=colors)
magaxis(c(2,3,4),labels=c(1,0,0),box=T)
axis(side=1,at=seq(from=1.5,to=34.5,by=2),labels=levels(T60$Taxa),las=2)
abline(v=16.5)

T60<-subset(gamfits2, Time==60)

fm1m60 <- lm(predMass~Taxa*Treatment,data=subset(T60, Location=="S" & Range=="narrow"))#, method="ML")
plot(fm1m60)
summary(fm1m60)  
fm1m60 <- lm(predMass~Taxa*Treatment,data=subset(T60, Location=="S" & Range=="wide"))#, method="ML")
plot(fm1m60)
summary(fm1m60)
fm1m60 <- lm(predMass~Taxa*Treatment,data=subset(T60, Location=="N" & Range=="narrow"))#, method="ML")
plot(fm1m60)
summary(fm1m60)  
fm1m60 <- lm(predMass~Taxa*Treatment,data=subset(T60, Location=="N" & Range=="wide"))#, method="ML")
plot(fm1m60)
summary(fm1m60)
...
#per provenance
provber<- summaryBy(predMass~Time+Taxa+Treatment, data=gamfits2, FUN=mean, keep.names=T)
provberw<-provber[provber$Treatment=="Warmed",]; provberh<-provber[ber$Treatment=="Home",]
provberw$RR<-(provberw$predMass-provberh$predMass)/provberh$predMass

provberw[ provberw$RR %in% tapply(provberw$RR, provberw$Taxa, max), ] #Max ratio 96.8 on Day 37 Wide, 47.9 on Day 45 Narrow
...
#Did SLA change?
#Canopy SLA
fm1SLA <- lme(log(Leafarea)~log(Leafmass)*Treatment*Range*Location,random=list(~1|Sp_RS_EN,~1|Prov_Sp_EN),data=data2)#subset(data2, Taxa!="SMIT"& SLA<350))#, method="ML")
